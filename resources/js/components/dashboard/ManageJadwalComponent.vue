<template id="jadwals-list">
<div class="contentlist">
<div class="flex-container">
      <div class="columns m-t-10">
        <div class="column">
          <h1 class="title">Manage Jadwal</h1>
        </div>
        <div class="column" style="height: 60px;">
          <!-- <button class="button is-success is-pulled-right" v-on:click="modalCreate"><i class="fa fa-user-plus m-r-10"></i> Buat Jadwal Baru</button> -->
        </div>
      </div>
      <div class="columns m-t-10">
        <div class="column">
        ...
        <div class="card-content">
          <table class="table is-hoverable is-striped">
            ...
            <tbody>
                ...
                <tr v-for="(jadwals, index) in filterAdmin " :key="jadwals.id">
                  <th>{{ index + 1 + start}}</th>
                  <td>{{ jadwals.jadwal }}</td>
                  <td>{{ jadwals.tempat }}</td>
                  <td>{{ jadwals.mulai }}</td>
                  <td>{{ jadwals.tanggal }}</td>
                  <td>{{ jadwals.created_at }}</td>
                  <td><a class="button is-danger" href="#" v-on:click.prevent="modalDelete();setIdDelete(jadwals)">Hapus</a></td>
                </tr>
        ...
        <div class="modal-card">
          <header class="modal-card-head">
            ...
            <div class="columns">
              <div class="column">
                <div class="field">
                  <label for="jadwal" class="label">Nama</label>
                  <p class="control">
                    <input type="text" class="input" v-model="dataAdmin.jadwal" jadwal="jadwal" id="jadwal" required>
                  </p>
                </div>
                <div class="field">
                  <label for="jadwal" class="label">Username</label>
                  <p class="control">
                    <input type="text" class="input" v-model="dataAdmin.username" jadwal="username" id="username" required>
                  </p>
                </div>
                ...
                <div class="field">
                  ...
                  <p class="control">
                    <input type="password" class="input" jadwal="password" v-model="dataAdmin.password" id="password" required >
                  </p>
                </div>
          ...
          <footer class="modal-card-foot">
            <button class="button is-success" :class="{'is-loading' : load}" @click="updateLoad()">Buat Jadwal</button>
    ...
    <div  class="modal" v-bind:class="{ 'is-active' : activeUpdate }">
      <div class="modal-background" v-on:click="modalUpdate()" ></div>
        ...
        <div class="modal-card">
          <header class="modal-card-head">
            <p class="modal-card-title">Perbaharui Jadwal</p>
            ...
            <div class="columns">
              <div class="column">
                ...
                <div class="field">
                  <label for="password" class="label">Password</label>
                  <p class="control">
                    <input type="password" class="input" jadwal="password" v-model="dataAdmin.password" id="password" >
                  </p>
                </div>
                <div class="field">
                  <p class="control">
    ...
    <div  class="modal" v-bind:class="{ 'is-active' : activeDelete }">
      <div class="modal-background" v-on:click="modalDelete()" ></div>
        ...
        <div class="modal-card">
          <header class="modal-card-head">
            <p class="modal-card-title">Hapus Jadwal</p>
            <button class="delete" aria-label="close" v-on:click="modalDelete()"></button>
          </header>
            <form v-on:submit.prevent ="deleteAdmin()">

            <section class="modal-card-body">
                <p>Jadwal yang dihapus tidak dapat dikembalikan lagi, apakah anda yakin ingin menghapusnya ? </p>
          </section>
          <footer class="modal-card-foot">
       ...
       components: {
        ...
        created: function() {
          if(localStorage.getItem('roles') == 'user'){
                  this.$router.push({ jadwal: 'DashboardContent' });
          }else{
            this.getAdmin();
          }
         ...
         methods: {
          ...
          updateLoad(){
            ...
            updateAdmin(){
              ...
              let uri = '/api/jadwals'+this.dataAdmin.id;
        ...
        computed: {
            ...
            //                 if(this.search != '') return row.jadwal.toLowerCase().includes(this.search.toLowerCase());
