<template id="catatans-list">
<div class="contentlist">
<div class="flex-container">
      <div class="columns m-t-10">
        <div class="column">
          <h1 class="title">Manage Catatan</h1>
        </div>
        <div class="column" style="height: 60px;">
          <!-- <button class="button is-success is-pulled-right" v-on:click="modalCreate"><i class="fa fa-user-plus m-r-10"></i> Buat Catatan Baru</button> -->
        </div>
      </div>
      <div class="columns m-t-10">
        <div class="column">
        ...
        <div class="card-content">
          <table class="table is-hoverable is-striped">
            ...
            <tbody>
                ...
                <tr v-for="(catatans, index) in filterAdmin " :key="catatans.id">
                  <th>{{ index + 1 + start}}</th>
                  <td>{{ catatans.catatan }}</td>
                  <td>{{ catatans.created_at }}</td>
                  <td><a class="button is-danger" href="#" v-on:click.prevent="modalDelete();setIdDelete(catatans)">Hapus</a></td>
                </tr>
      ...
      <div class="modal-background" v-on:click="modalCreate" ></div>
        <div class="modal-card">
          <header class="modal-card-head">
            <p class="modal-card-title">Tambahkan Catatan</p>
            ...
            <div class="columns">
              <div class="column">
                <div class="field">
                  <label for="catatan" class="label">Nama</label>
                  <p class="control">
                    <input type="text" class="input" v-model="dataAdmin.catatan" catatan="catatan" id="catatan" required>
                  </p>
                </div>
                <div class="field">
                  <label for="catatan" class="label">Username</label>
                  <p class="control">
                    <input type="text" class="input" v-model="dataAdmin.username" catatan="username" id="username" required>
                  </p>
                </div>
                ...
                <div class="field">
                  ...
                  <p class="control">
                    <input type="password" class="input" catatan="password" v-model="dataAdmin.password" id="password" required >
                  </p>
                </div>
          ...
          <footer class="modal-card-foot">
            <button class="button is-success" :class="{'is-loading' : load}" @click="updateLoad()">Buat Catatan</button>
    ...
    <div  class="modal" v-bind:class="{ 'is-active' : activeUpdate }">
      <div class="modal-background" v-on:click="modalUpdate()" ></div>
        ...
        <div class="modal-card">
          <header class="modal-card-head">
            <p class="modal-card-title">Perbaharui Catatan</p>
            ...
            <div class="columns">
              <div class="column">
                ...
                <div class="field">
                  <label for="password" class="label">Password</label>
                  <p class="control">
                    <input type="password" class="input" catatan="password" v-model="dataAdmin.password" id="password" >
                  </p>
                </div>
                <div class="field">
                  <p class="control">
    ...
    <div  class="modal" v-bind:class="{ 'is-active' : activeDelete }">
      <div class="modal-background" v-on:click="modalDelete()" ></div>
        ...
        <div class="modal-card">
          <header class="modal-card-head">
            <p class="modal-card-title">Hapus Catatan</p>
            <button class="delete" aria-label="close" v-on:click="modalDelete()"></button>
          </header>
            <form v-on:submit.prevent ="deleteAdmin()">

            <section class="modal-card-body">
                <p>Catatan yang dihapus tidak dapat dikembalikan lagi, apakah anda yakin ingin menghapusnya ? </p>
          </section>
          <footer class="modal-card-foot">
       ...
       components: {
        ...
        created: function() {
          if(localStorage.getItem('roles') == 'user'){
                  this.$router.push({ catatan: 'Landing' });
          }else{
            this.getAdmin();
          }
         ...
         methods: {
          ...
          updateLoad(){
            ...
            updateAdmin(){
              ...
              let uri = '/api/catatan/'+this.dataAdmin.id;
        ...
        computed: {
            ...
            //                 if(this.search != '') return row.catatan.toLowerCase().includes(this.search.toLowerCase());
